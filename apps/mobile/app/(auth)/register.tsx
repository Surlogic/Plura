...
export default function RegisterScreen() {
  ...
  const { googleRequest, isGoogleSubmitting, handleGoogleAuth } = useGoogleOAuth({
    ...
    authAction: 'REGISTER',
    refreshProfile,
    onSuccess: continueAfterRegister,
    onError: (message) => {
  ...
  const loadingTitle = 'Registrando cuenta';
  const loadingDescription = isGoogleSubmitting
    ? `Creando tu cuenta con Google como ${role}.`
    : role === 'profesional'
      ? 'Guardando tus datos y validando la ubicación de tu perfil profesional.'
      : 'Creando tu cuenta de cliente en Plura.';
            ...
            <View className="w-full space-y-4 rounded-[32px] bg-white p-8 shadow-sm">
              ...
              <View className="space-y-4">
                <View className="space-y-3">
                  ...
                  <TouchableOpacity
                    className="h-14 w-full items-center justify-center rounded-full border border-secondary/15 bg-white"
