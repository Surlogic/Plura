...
import { ActivityIndicator, Alert, Linking, Switch, Text, TextInput, TouchableOpacity, View } from 'react-native';
...
export default function ClientSettingsScreen() {
  const { clientProfile, hasLoaded, isAuthenticated, refreshProfile, logout } = useClientSession();
  ...
  const handleSendPhoneVerification = async () => {
    if (isSendingPhoneVerification || !clientProfile?.phoneNumber) return;
  ...
  const handleConfirmPhoneVerification = async () => {
    if (isConfirmingPhoneVerification || !clientProfile?.phoneNumber) return;
    setPhoneVerificationMessage(null);
    setPhoneVerificationError(null);
