import { useCallback, useEffect, useState } from 'react';
import {
  createBookingReview,
  deleteBookingReview,
  getBookingReview,
  getReviewEligibility,
} from '@/services/clientReviews';
import type { BookingReviewResponse, ReviewEligibilityResponse } from '@/types/review';

type Props = {
  bookingId: string;
  onReviewCreated?: () => void;
  onReviewDeleted?: () => void;
};

const StarSelector = ({
  value,
  onChange,
  disabled,
}: {
  value: number;
  onChange: (v: number) => void;
  disabled: boolean;
}) => {
  const [hover, setHover] = useState(0);

  return (
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map((star) => (
        <button
          key={star}
          type="button"
          disabled={disabled}
          onMouseEnter={() => setHover(star)}
          onMouseLeave={() => setHover(0)}
          onClick={() => onChange(star)}
          className="text-2xl transition disabled:cursor-not-allowed"
          aria-label={`Calificar con ${star} estrella${star > 1 ? 's' : ''}`}
        >
          <span className={star <= (hover || value) ? 'text-[#F59E0B]' : 'text-[#CBD5E1]'}>
            ★
          </span>
        </button>
      ))}
    </div>
  );
};

const StarSummary = ({ rating }: { rating: number }) => (
  <span className="text-lg">
    <span className="text-[#F59E0B]">{'★'.repeat(rating)}</span>
    <span className="text-[#CBD5E1]">{'★'.repeat(5 - rating)}</span>
  </span>
);

export default function ClientBookingReviewSection({
  bookingId,
  onReviewCreated,
  onReviewDeleted,
}: Props) {
  const [eligibility, setEligibility] = useState<ReviewEligibilityResponse | null>(null);
  const [existingReview, setExistingReview] = useState<BookingReviewResponse | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [rating, setRating] = useState(0);
  const [text, setText] = useState('');
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isDeleting, setIsDeleting] = useState(false);
  const [loadError, setLoadError] = useState<string | null>(null);
  const [submitError, setSubmitError] = useState<string | null>(null);
  const [success, setSuccess] = useState(false);

  const load = useCallback(async (isActive?: () => boolean) => {
    setIsLoading(true);
    setLoadError(null);

    try {
      const elig = await getReviewEligibility(bookingId);
      if (isActive && !isActive()) return;

      setEligibility(elig);

      if (elig.alreadyReviewed) {
        const lookup = await getBookingReview(bookingId);
        if (isActive && !isActive()) return;
        setExistingReview(lookup.exists ? lookup.review : null);
      } else {
        setExistingReview(null);
      }
    } catch (error) {
      if (isActive && !isActive()) return;
      setEligibility(null);
      setExistingReview(null);
      const message =
        error instanceof Error ? error.message : 'No pudimos cargar la sección de reseñas.';
      setLoadError(message);
    } finally {
      if (!isActive || isActive()) {
        setIsLoading(false);
      }
    }
  }, [bookingId]);

  useEffect(() => {
    let active = true;
    void load(() => active);
    return () => {
      active = false;
    };
  }, [load]);

  const handleSubmit = async () => {
    if (rating === 0) {
      setSubmitError('Elegí una calificación.');
      return;
    }

    setIsSubmitting(true);
    setSubmitError(null);

    try {
      const review = await createBookingReview(bookingId, {
        rating,
        text: text.trim() || null,
      });
      setExistingReview(review);
      setEligibility((prev) =>
        prev
          ? {
              ...prev,
              eligible: false,
              alreadyReviewed: true,
            }
          : prev,
      );
      setSuccess(true);
      setText('');
      onReviewCreated?.();
    } catch (error) {
      const message =
        error instanceof Error ? error.message : 'No pudimos enviar tu reseña.';
      setSubmitError(message);
    } finally {
      setIsSubmitting(false);
    }
  };

  const handleDelete = async () => {
    setIsDeleting(true);
    setSubmitError(null);

    try {
      await deleteBookingReview(bookingId);
      setExistingReview(null);
      setRating(0);
      setText('');
      setSuccess(false);
      await load();
      onReviewDeleted?.();
    } catch (error) {
      const message =
        error instanceof Error ? error.message : 'No pudimos eliminar tu reseña.';
      setSubmitError(message);
    } finally {
      setIsDeleting(false);
    }
  };

  if (isLoading) {
    return (
      <div className="mt-4 rounded-[18px] border border-[#E2E7EC] bg-white p-4">
        <p className="text-sm text-[#64748B]">Cargando sección de reseñas...</p>
      </div>
    );
  }

  if (loadError) {
    return (
      <div className="mt-4 rounded-[18px] border border-[#FECACA] bg-[#FEF2F2] p-4">
        <p className="text-sm font-semibold text-[#991B1B]">No pudimos cargar las reseñas</p>
        <p className="mt-1 text-sm text-[#B91C1C]">{loadError}</p>
      </div>
    );
  }

  if (!eligibility) return null;

  if (existingReview) {
    return (
      <div className="mt-4 rounded-[18px] border border-[#E2E7EC] bg-white p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xs uppercase tracking-[0.25em] text-[#94A3B8]">Tu reseña</p>
            <div className="mt-3 flex items-center gap-2">
              <StarSummary rating={existingReview.rating} />
              <span className="text-sm font-semibold text-[#0E2A47]">{existingReview.rating}/5</span>
            </div>
          </div>

          <button
            type="button"
            onClick={() => void handleDelete()}
            disabled={isDeleting}
            className="shrink-0 rounded-full border border-[#FECACA] bg-white px-3 py-1.5 text-xs font-semibold text-[#B91C1C] transition hover:-translate-y-0.5 hover:shadow-sm disabled:cursor-not-allowed disabled:opacity-50"
          >
            {isDeleting ? 'Eliminando...' : 'Eliminar reseña'}
          </button>
        </div>

        {existingReview.text ? (
          <p className="mt-2 text-sm text-[#475569]">{existingReview.text}</p>
        ) : (
          <p className="mt-2 text-xs italic text-[#94A3B8]">Sin comentario, solo calificación.</p>
        )}

        {submitError ? (
          <p className="mt-2 text-xs text-[#B91C1C]">{submitError}</p>
        ) : null}

        {success ? (
          <p className="mt-2 text-xs text-[#0F766E]">Reseña enviada correctamente.</p>
        ) : null}
      </div>
    );
  }

  if (!eligibility.eligible) return null;

  return (
    <div className="mt-4 rounded-[18px] border border-[#BFEDE7] bg-[#F0FDFA] p-4">
      <p className="text-xs uppercase tracking-[0.25em] text-[#94A3B8]">Dejá tu reseña</p>
      <p className="mt-1 text-sm text-[#475569]">
        Tu opinión ayuda a otros clientes a elegir mejor.
      </p>

      <div className="mt-3">
        <StarSelector value={rating} onChange={setRating} disabled={isSubmitting || isDeleting} />
      </div>

      <textarea
        value={text}
        onChange={(e) => setText(e.target.value)}
        placeholder="Contanos tu experiencia (opcional)"
        maxLength={2000}
        disabled={isSubmitting || isDeleting}
        className="mt-3 min-h-20 w-full rounded-[14px] border border-[#D9E2EC] bg-white px-3 py-2 text-sm text-[#0E2A47] outline-none transition focus:border-[#1FB6A6] disabled:opacity-60"
      />

      {submitError ? (
        <p className="mt-2 text-xs text-[#B91C1C]">{submitError}</p>
      ) : null}

      <button
        type="button"
        onClick={handleSubmit}
        disabled={isSubmitting || isDeleting || rating === 0}
        className="mt-3 w-full rounded-full bg-[#0B1D2A] px-4 py-2.5 text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:shadow-md disabled:cursor-not-allowed disabled:opacity-60"
      >
        {isSubmitting ? 'Enviando...' : 'Enviar reseña'}
      </button>
    </div>
  );
}
