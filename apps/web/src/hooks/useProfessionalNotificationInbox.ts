...
} from '@/services/professionalNotifications';
import type {
...
const dedupeNotifications = (
  current: ProfessionalNotificationItem[],
  incoming: ProfessionalNotificationItem[],
) => {
...
export const useProfessionalNotificationInbox = () => {
  const { publishChange } = useProfessionalNotificationsContext();
  const mountedRef = useRef(true);
  const requestIdRef = useRef(0);
  const itemsRef = useRef<ProfessionalNotificationItem[]>([]);
    ...
    async ({ append = false }: { append?: boolean } = {}) => {
      ...
      try {
        ...
        setError(
          extractApiMessage(
            requestError,
            'No se pudieron cargar las notificaciones del profesional.',
          ),
        );
        if (!append) {
          setItems([]);
  ...
  const setStatusFilter = useCallback((status: ProfessionalNotificationStatus) => {
    setFilters((current) => ({
      ...current,
      status,
    }));
