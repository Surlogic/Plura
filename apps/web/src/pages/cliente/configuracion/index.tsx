...
export default function ClienteConfiguracionPage() {
  ...
  const handleDeleteAccount = async () => {
    ...
    if (!deleteChallengeId || !deleteChallengeCode.trim()) {
      setDeleteError('Primero enviá y validá el código recibido por email.');
      return;
    }
  ...
  const handleSendDeleteChallenge = async () => {
    ...
    try {
      ...
      setDeleteError(resolveBackendMessage(error, 'No se pudo enviar el código por email.'));
    } finally {
  ...
  const handleVerifyDeleteChallenge = async () => {
    ...
    if (!deleteChallengeId) {
      setDeleteError('Primero enviá el código por email.');
      return;
    }
    if (!deleteChallengeCode.trim()) {
      setDeleteError('Ingresá el código recibido para validarlo.');
      return;
    }
    ...
    try {
      ...
      setIsDeleteChallengeVerified(false);
      setDeleteError(resolveBackendMessage(error, 'No se pudo validar el código.'));
    } finally {
            ...
            <div className="mt-5 space-y-4">
              ...
              <div className="grid gap-3 md:grid-cols-3">
                <div className="rounded-[18px] border border-[color:var(--border-soft)] bg-[color:var(--surface-soft)] p-4">
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[color:var(--ink-faint)]">Paso 1</p>
                  <p className="mt-1 text-sm font-semibold text-[color:var(--ink)]">Enviar código</p>
                  <p className="mt-1 text-xs text-[color:var(--ink-muted)]">
                ...
                <div className="rounded-[18px] border border-[color:var(--border-soft)] bg-[color:var(--surface-soft)] p-4">
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[color:var(--ink-faint)]">Paso 2</p>
                  <p className="mt-1 text-sm font-semibold text-[color:var(--ink)]">Validar código</p>
                  <input
                    type="text"
                    inputMode="numeric"
                    pattern="[0-9]*"
                    ...
                    className="mt-3 h-11 w-full rounded-[16px] border border-[color:var(--error-soft)] bg-[color:var(--surface-strong)] px-4 text-sm text-[color:var(--ink)] focus:border-[color:var(--error)] focus:outline-none focus:ring-4 focus:ring-[color:var(--error-soft)] disabled:cursor-not-allowed disabled:opacity-60"
                  />
                  <button
                    type="button"
                    onClick={() => {
              ...
              {deleteChallengeMessage ? (
                <p className="text-xs font-semibold text-[color:var(--warning)]">{deleteChallengeMessage}</p>
              ) : null}
              {deleteSuccessMessage ? (
                <p className="text-xs font-semibold text-[color:var(--success)]">{deleteSuccessMessage}</p>
              ) : null}
              {deleteError ? (
                <p className="text-xs font-semibold text-[color:var(--error)]">{deleteError}</p>
              ) : null}
            </div>
          ) : null}
        </article>
