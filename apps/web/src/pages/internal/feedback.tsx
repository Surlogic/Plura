  ...
  return (
    <div className="mx-auto max-w-md space-y-4 rounded-[20px] border border-[#E2E7EC] bg-white p-6 shadow-sm">
      ...
      <div>
        ...
        <input type="text" value={apiUrl} onChange={(e) => setApiUrl(e.target.value)} placeholder="http://localhost:3000" className="mt-1 w-full rounded-[12px] border border-[#D9E2EC] px-3 py-2 text-sm" />
      </div>
      <div>
        <label className="text-xs font-semibold text-[#64748B]">X-Internal-Token</label>
        <input type="password" value={token} onChange={(e) => setToken(e.target.value)} placeholder="Token interno" className="mt-1 w-full rounded-[12px] border border-[#D9E2EC] px-3 py-2 text-sm" />
      </div>
      <button
        type="button"
        onClick={() => { localStorage.setItem('plura_ops_api_url', apiUrl.trim()); localStorage.setItem('plura_ops_token', token.trim()); onConfigured(); }}
...
function FeedbackTab() {
  ...
  const load = useCallback(async (pageNum: number) => {
    ...
    try {
      const [listResult, analyticsResult] = await Promise.all([
        fetchFeedbackList({ page: pageNum, size: 20, authorRole: filterRole || undefined, category: filterCategory || undefined, rating: filterRating || undefined, status: filterStatus || undefined, from: filterFrom || undefined, to: filterTo || undefined }),
  ...
  const handleArchiveToggle = async (id: number, currentStatus: string) => {
    try {
      if (currentStatus === 'ARCHIVED') { await unarchiveFeedback(id); } else { await archiveFeedback(id); }
  ...
  return (
    <div className="space-y-5">
      {error ? <div className="rounded-[14px] border border-[#FECACA] bg-[#FFF5F5] px-4 py-3 text-sm text-[#B91C1C]">{error}</div> : null}
...
function ReviewsTab() {
  ...
  const handleHideText = async (id: number) => {
    setActionLoading(id);
    try { await hideReviewTextOps(id); await load(currentPage); } catch (e) { setError(e instanceof Error ? e.message : 'Error.'); } finally { setActionLoading(null); }
  };

  const handleShowText = async (id: number) => {
    setActionLoading(id);
    try { await showReviewTextOps(id); await load(currentPage); } catch (e) { setError(e instanceof Error ? e.message : 'Error.'); } finally { setActionLoading(null); }
  };
  ...
  const tabClass = (tab: string) =>
    `rounded-full px-4 py-2 text-sm font-semibold transition ${
      activeTab === tab
        ...
        : 'bg-white text-[#475569] border border-[#E2E7EC] hover:bg-[#F1F5F9]'
    }`;

  return (
    <>
      <Head>
        <title>Panel interno | Plura Ops</title>
      ...
      <div className="min-h-screen bg-[#F8FAFC] px-4 py-8">
        <div className="mx-auto max-w-5xl space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-semibold text-[#0E2A47]">Panel interno</h1>
              <p className="text-sm text-[#64748B]">Operaciones internas de Plura</p>
            </div>
            {configured ? (
              <button
                type="button"
                onClick={() => { localStorage.removeItem('plura_ops_api_url'); localStorage.removeItem('plura_ops_token'); setConfigured(false); }}
          ...
          {!configured ? (
            ...
            <>
              ...
              {activeTab === 'feedback' ? <FeedbackTab /> : <ReviewsTab />}
            </>
          )}
        </div>
      </div>
