...
/**
 * AppFeedbackRepository es un contrato interno del modulo feedback / persistencia.
...
public interface AppFeedbackRepository extends JpaRepository<AppFeedback, Long> {
    ...
    @Query(
        value = """
            ...
            FROM AppFeedback f
            JOIN FETCH f.author
            WHERE f.status = com.plura.plurabackend.core.feedback.model.FeedbackStatus.ACTIVE
              AND f.publicVisible = TRUE
    ...
    Page<AppFeedback> findPublicVisible(Pageable pageable);
