...
public class BookingReviewService {
    ...
    public BookingReviewResponse createReview(Long bookingId, Long clientUserId, CreateBookingReviewRequest request) {
        ...
        try {
            ...
            LOGGER.warn("Failed to send REVIEW_RECEIVED notification for reviewId={}: {}",
        ...
        return toResponse(review, booking.getUser().getFullName());
    }
    ...
    public Optional<BookingReviewResponse> getReviewByBookingId(Long bookingId, Long clientUserId) {
        ...
        return bookingReviewRepository.findByBooking_Id(bookingId)
            .map(review -> toResponse(review, booking.getUser().getFullName()));
    }
    ...
    public Page<BookingReviewResponse> listProfessionalReviews(Long professionalUserId, Pageable pageable) {
        ...
        return bookingReviewRepository.findProfessionalDashboardReviews(professionalId, pageable)
            .map(review -> toProfessionalResponse(review));
    }
    ...
    public void hideReviewText(Long reviewId, Long professionalUserId) {
        ...
        if (!professionalId.equals(review.getProfessional().getId())) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "No autorizado");
        }
    ...
    /**
     * Recomputes rating/reviewsCount for every professional that has at least one review
     * in a single batch UPDATE instead of N individual queries.
     * Safe to call on deployment or as a one-off backfill if aggregates are stale.
     */
    @Transactional
    ...
    private BookingReviewResponse toResponse(BookingReview review, String authorDisplayName) {
    ...
    private BookingReviewResponse toProfessionalResponse(BookingReview review) {
