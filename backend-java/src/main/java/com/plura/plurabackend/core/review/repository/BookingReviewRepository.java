...
/**
 * BookingReviewRepository es un contrato interno del modulo resenas / persistencia.
...
public interface BookingReviewRepository extends JpaRepository<BookingReview, Long> {
    ...
    @Query(
        ...
        LEFT JOIN FETCH r.businessRepliedByUser replyUser
        WHERE r.id = :reviewId
          AND r.professional.id = :professionalId
        """
    )
    Optional<BookingReview> findDetailedByIdAndProfessionalId(
