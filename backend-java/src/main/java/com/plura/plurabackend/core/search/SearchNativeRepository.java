...
public class SearchNativeRepository {
    ...
    public SearchPageResult search(
        ...
        String selectSql =
            "SELECT "
                ...
                + "doc.display_name AS business_name, "
                ...
                + "  ELSE " + buildSearchRelevanceExpression("doc") + " "
                + "END AS relevance "
    ...
    private String buildSearchWhereClause(String dateMatchExpression, String availableNowMatchExpression) {
        return " WHERE 1 = 1 "
            ...
            + " )) "
            + " AND (:queryBlank = true OR ("
            + "   " + buildSearchRelevanceExpression("doc") + " >= :similarityThreshold "
    ...
    private String buildSearchRelevanceExpression(String alias) {
        return "CASE "
            ...
            + "    similarity(" + alias + ".rubro_normalized, :queryNormalized), "
            + "    similarity(" + alias + ".category_names_normalized, :queryNormalized), "
            + "    similarity(" + alias + ".service_category_names_normalized, :queryNormalized), "
            + "    COALESCE(ts_rank_cd(" + alias + ".search_vector, plainto_tsquery('simple', :queryTs)), 0.0)"
            + "  ) "
            + "  WHEN :type = 'PROFESIONAL' THEN GREATEST("
            + "    similarity(" + alias + ".name_normalized, :queryNormalized), "
            + "    similarity(" + alias + ".headline_normalized, :queryNormalized), "
            + "    COALESCE(ts_rank_cd(" + alias + ".search_vector, plainto_tsquery('simple', :queryTs)), 0.0)"
            + "  ) "
            + "  WHEN :type = 'LOCAL' THEN GREATEST("
            + "    similarity(" + alias + ".name_normalized, :queryNormalized), "
            + "    similarity(" + alias + ".headline_normalized, :queryNormalized), "
            + "    similarity(" + alias + ".location_text_normalized, :queryNormalized), "
            + "    COALESCE(ts_rank_cd(" + alias + ".search_vector, plainto_tsquery('simple', :queryTs)), 0.0)"
            + "  ) "
            + "  WHEN :type = 'SERVICIO' THEN GREATEST("
            + "    similarity(" + alias + ".service_names_normalized, :queryNormalized), "
            + "    similarity(" + alias + ".name_normalized, :queryNormalized), "
            + "    similarity(" + alias + ".rubro_normalized, :queryNormalized), "
            + "    similarity(" + alias + ".service_category_names_normalized, :queryNormalized), "
            + "    COALESCE(ts_rank_cd(" + alias + ".search_vector, plainto_tsquery('simple', :queryTs)), 0.0)"
