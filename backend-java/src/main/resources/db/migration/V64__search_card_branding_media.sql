DROP MATERIALIZED VIEW IF EXISTS search_professional_document_mv;

CREATE MATERIALIZED VIEW search_professional_document_mv AS
...
service_agg AS (
    SELECT
        ...
        (array_agg(NULLIF(s.image_url, '') ORDER BY s.created_at, s.id))[1] AS first_service_image_url
...
legacy_photo_agg AS (
    SELECT
        ...
        (array_agg(NULLIF(photo.url, '') ORDER BY photo.position))[1] AS fallback_photo_url
    FROM professional_profile_photos photo
    GROUP BY photo.professional_id
),
business_photo_agg AS (
    SELECT
        photo.professional_id,
        (array_agg(NULLIF(photo.url, '') ORDER BY photo.created_at, photo.id))[1] AS fallback_photo_url
    FROM business_photo photo
    WHERE photo.type IN ('LOCAL', 'WORK')
    GROUP BY photo.professional_id
)
SELECT
    ...
    COALESCE(NULLIF(p.banner_url, ''), business_photo_agg.fallback_photo_url, legacy_photo_agg.fallback_photo_url, service_agg.first_service_image_url) AS cover_image_url,
    NULLIF(p.banner_url, '') AS banner_url,
    COALESCE(p.banner_position_x, 50)::double precision AS banner_position_x,
    COALESCE(p.banner_position_y, 50)::double precision AS banner_position_y,
    COALESCE(p.banner_zoom, 1)::double precision AS banner_zoom,
    NULLIF(p.logo_url, '') AS logo_url,
    COALESCE(p.logo_position_x, 50)::double precision AS logo_position_x,
    COALESCE(p.logo_position_y, 50)::double precision AS logo_position_y,
    COALESCE(p.logo_zoom, 1)::double precision AS logo_zoom,
    COALESCE(business_photo_agg.fallback_photo_url, legacy_photo_agg.fallback_photo_url) AS fallback_photo_url,
...
LEFT JOIN legacy_photo_agg ON legacy_photo_agg.professional_id = p.id
LEFT JOIN business_photo_agg ON business_photo_agg.professional_id = p.id
WHERE p.active = true;

CREATE UNIQUE INDEX idx_search_professional_document_mv_professional_id
    ON search_professional_document_mv (professional_id);
CREATE INDEX idx_search_professional_document_mv_geom
    ON search_professional_document_mv USING gist (geom);
CREATE INDEX idx_search_professional_document_mv_category_slugs
    ON search_professional_document_mv USING gin (category_slugs);
CREATE INDEX idx_search_professional_document_mv_name_trgm
    ON search_professional_document_mv USING gin (name_normalized gin_trgm_ops);
CREATE INDEX idx_search_professional_document_mv_headline_trgm
    ON search_professional_document_mv USING gin (headline_normalized gin_trgm_ops);
CREATE INDEX idx_search_professional_document_mv_rubro_trgm
    ON search_professional_document_mv USING gin (rubro_normalized gin_trgm_ops);
CREATE INDEX idx_search_professional_document_mv_location_trgm
    ON search_professional_document_mv USING gin (location_text_normalized gin_trgm_ops);
CREATE INDEX idx_search_professional_document_mv_categories_trgm
    ON search_professional_document_mv USING gin (category_names_normalized gin_trgm_ops);
CREATE INDEX idx_search_professional_document_mv_services_trgm
    ON search_professional_document_mv USING gin (service_names_normalized gin_trgm_ops);
CREATE INDEX idx_search_professional_document_mv_service_categories_trgm
    ON search_professional_document_mv USING gin (service_category_names_normalized gin_trgm_ops);
CREATE INDEX idx_search_professional_document_mv_search_vector
