...
- el wizard web de registro profesional cierra con activacion de `Plura Core`: crea o completa la cuenta base profesional solo en el submit final, inicia billing para la prueba gratuita y recien cuando Core queda habilitado aplica la configuracion inicial de perfil/horarios; los servicios se cargan despues desde el dashboard/onboarding operativo. Si Mercado Pago devuelve checkout, redirige en la misma pestaña y conserva el handoff pendiente hasta la confirmacion; la verificacion puede recuperar la suscripcion por referencia aunque el email pagador de Mercado Pago sea distinto, por `providerSubscriptionId` si vuelve en la URL, por email + plan si Mercado Pago no conserva la referencia, o por plan + ventana temporal del checkout si tampoco coincide el email
...
- autenticacion y seguridad: registro, login, recuperacion, sesiones y OAuth; hoy web y mobile exponen Google como login social activo, mientras backend conserva soporte OAuth mas amplio a nivel modulo auth; conviven dos recuperaciones de contraseña (`/auth/password/forgot` + `/auth/password/reset` como flujo legacy por token y `/auth/password/recovery/*` como flujo escalonado email + telefono + OTP por email); al confirmar cualquiera de los resets, backend devuelve el `role` recuperado (`USER` o `PROFESSIONAL`) para redirigir al login correcto y limpiar sesion previa; el backend ya autoriza superficies cliente/profesional por contexto activo del JWT (`ctx=CLIENT` o `ctx=PROFESSIONAL`) y no por `UserRole` legacy: `CLIENT` depende de `app_user.client_active=true` y `PROFESSIONAL` exige `ProfessionalProfile.active=true`; una cuenta autenticada puede activar o reactivar su capacidad profesional con `POST /auth/professional-profile/activate` sin crear otro `app_user` ni tomar control de emails ajenos; el registro por email cliente ya soporta OTP SMS unico previo al alta con Vonage Verify API y, al activarlo por env, deja el telefono verificado desde el inicio; el registro profesional guarda el telefono como contacto sin exigir OTP en el wizard; login OAuth puede requerir completar telefono via `POST /auth/oauth/complete-phone`; cierre de perfil profesional usa `DELETE /auth/professional-profile` desde `ctx=PROFESSIONAL`, cierre de faceta cliente usa `DELETE /auth/client-profile` desde `ctx=CLIENT` cuando queda otro contexto activo, y ninguno elimina la otra faceta; eliminacion total de cuenta usa `DELETE /auth/me` con `scope=TOTAL`, `challengeId` y `code`
...
- la web ahora persiste solo un `session hint` con rol (`CLIENT` o `PROFESSIONAL`) para poder rehidratar el perfil correcto tambien en `/` y otras rutas publicas despues de cerrar y reabrir el navegador; el access token queda solo en memoria y se recupera por refresh/cookie, no en localStorage
- web y mobile ya priorizan contexto/intencion por sobre `role` legacy en los accesos principales: login cliente y reserva usan `/auth/login` o `/auth/context/select` con `ctx=CLIENT`; mobile cliente/profesional tambien pide `/auth/login` con `desiredContext`; `/login` muestra selector cuando la cuenta tiene mas de un contexto incluso si llego desde `Soy profesional`, y el onboarding profesional solo fuerza activacion de `ProfessionalProfile` con `POST /auth/professional-profile/activate` cuando la cuenta autenticada aun no tiene ese contexto
...
- al cerrar sesion desde la web, la UI ahora muestra un overlay transitorio de `Cerrando sesión` y redirige al login unificado `/login` en vez de dejar la pantalla sin feedback mientras limpia estado local
...
- Regla de seguridad: los clientes pueden navegar e iniciar el flujo, pero no pueden confirmar reservas sin celular verificado (`phoneVerified=true`); el wizard profesional permite guardar telefono de contacto sin OTP y deja la verificacion pendiente para una superficie posterior.
...
- Profesional/local: el registro profesional guarda el telefono como contacto sin OTP en el wizard; la verificacion telefonica profesional queda pendiente para una superficie posterior. Cambiar el teléfono verificado limpia la verificación hasta completar una nueva validación.
