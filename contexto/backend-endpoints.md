...
- `POST /auth/register/cliente` y `POST /auth/register/profesional` aceptan `phoneVerificationToken`, pero el registro cliente y el alta profesional no lo exigen. Si el token llega valido, el telefono queda verificado; si no llega, el telefono queda guardado sin `phoneVerifiedAt`. El telefono cliente y el telefono profesional pueden ser distintos en la misma cuenta y se validan por contexto, no de forma global.
- Regla vigente de verificación celular: el alta cliente no pide OTP SMS; `POST /auth/register/profesional` tampoco lo exige desde el wizard profesional y deja la verificacion pendiente. El telefono verificable por `/auth/verify/phone/send` es `app_user.phone_number`; en alta/activacion profesional se persiste ahi el telefono de registro cuando falta, sin pisar `professional_profile.whatsapp`, y un `phoneVerificationToken` válido deja `phoneVerifiedAt` seteado. Temporalmente, para testing, `POST /public/profesionales/{slug}/reservas` no bloquea por telefono no verificado salvo que se active `APP_BOOKING_REQUIRE_PHONE_VERIFICATION=true`.
- `POST /auth/oauth/complete-phone` cierra el faltante de telefono cuando el alta/login OAuth no lo trae; para contexto cliente no exige OTP y acepta `phoneVerificationToken` opcional para dejar el telefono verificado en el mismo paso
