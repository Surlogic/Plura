...
- cuando el refund queda iniciado pero pendiente de acreditacion, backend registra `PAYMENT_REFUND_PENDING` para profesional y cliente, lo refleja en inbox/timeline y genera email transaccional con leyenda de tiempos de Mercado Pago
- cuando el refund queda aprobado en el mismo dispatch provider, backend registra tambien `PAYMENT_REFUNDED` en ese momento; ya no depende exclusivamente de un webhook posterior para avisar por inbox/email
