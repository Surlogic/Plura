...
- `POST /auth/register/cliente` — crea contexto cliente nuevo o, si el email existe solo como profesional, agrega `CLIENT` al mismo `app_user` validando la password de la cuenta base. Bloquea si ese email ya tiene contexto cliente activo. El telefono cliente se guarda en `app_user.phone_number`, se valida solo contra otros clientes activos y exige `phoneVerificationToken` cuando `AUTH_REGISTRATION_PHONE_VERIFICATION_REQUIRED=true`; con el default `false`, el token es opcional y si llega valido marca el telefono como verificado.
- `POST /auth/register/profesional`
- `POST /auth/register/availability` — endpoint publico para validacion temprana del wizard de registro; acepta `email?`, `phoneNumber?` y `desiredContext?` (`CLIENT|PROFESSIONAL`). Con contexto, `emailAvailable` solo bloquea si la cuenta ya tiene ese mismo contexto y `phoneAvailable` valida telefono dentro del mismo contexto. Sin `desiredContext` mantiene validacion legacy global.
- `POST /auth/register/phone/send` — inicia OTP SMS opcional previo/alrededor del alta usando Vonage Verify API
- `POST /auth/register/phone/confirm` — valida OTP SMS y devuelve `verificationToken` corto opcional para adjuntar al submit final del registro y marcar el telefono verificado
- los registros mantienen un solo usuario base activo por email. El telefono es por contexto: `app_user.phone_number` identifica telefono cliente activo y `professional_profile.whatsapp` identifica telefono profesional activo. `V92` agrega indices unicos parciales por telefono cliente activo y telefono profesional activo.
...
- `POST /auth/client-profile/activate` — endpoint autenticado para sumar `CLIENT` al `app_user` actual cuando la cuenta ya tiene contexto profesional activo. Body: `{ "phoneNumber": "..." }`. No acepta email, password ni OTP, no crea otro usuario, guarda el telefono cliente en `app_user.phone_number`, valida que no pertenezca a otro cliente activo, no pisa `professional_profile.whatsapp` y devuelve `AuthMeResponse` con `contexts` actualizados. Si el usuario ya tiene `CLIENT` activo responde de forma idempotente.
...
- `POST /auth/oauth` — login/registro OAuth Google/Apple. Para `authAction=REGISTER`, `desiredRole` es obligatorio (`USER` o `PROFESSIONAL`); si falta, responde `400` y no crea usuario. Con `desiredRole=PROFESSIONAL` no crea `User` ni `ProfessionalProfile`: devuelve identidad verificada y `oauthRegistrationToken` temporal para continuar el wizard profesional hasta `/auth/register/profesional`. Si el email Google ya existe solo como cliente, tambien devuelve ese token pendiente para sumar `PROFESSIONAL` sobre el mismo `app_user`; si ya tiene profesional activo responde conflicto. Para `authAction=LOGIN` acepta `desiredContext` opcional (`CLIENT|PROFESSIONAL|WORKER`) y devuelve el contrato unificado de `/auth/login` (`activeContext`, `contexts`, `contextSelectionRequired`); si no llega `desiredContext` y hay varios contextos, no elige profesional por defecto y deja que frontend muestre selector. Si el contexto pedido no existe, responde `400` sin caer a otro dashboard. OAuth no marca el email como verificado por codigo.
- `POST /auth/oauth/complete-phone` — completa telefono faltante despues de OAuth. Si `AUTH_REGISTRATION_PHONE_VERIFICATION_REQUIRED=true`, exige `phoneVerificationToken`; con el default `false`, el token es opcional y solo marca `phoneVerifiedAt` cuando llega valido.
...
- `emailVerified` en respuestas `/auth/me/*` solo es `true` cuando existe un challenge de email confirmado por codigo para el email actual y el mismo `emailVerifiedAt`; cuentas OAuth o legacy con timestamp previo pueden pedir y confirmar codigo igual.
...
- `POST /api/v1/billing/professional-registration/checkout` — endpoint publico versionado para iniciar Mercado Pago desde el wizard profesional antes de crear cuenta/contexto/perfil profesional. Request: `planCode=PLAN_CORE`, `email`, `returnUrl?`. Si el email ya tiene perfil profesional activo devuelve `409` y no inicia Mercado Pago. Crea un intent durable `professional_registration_checkout_intent` con `checkoutRef` opaco, email normalizado, plan, referencia de registro, plan remoto, URL de checkout, estado interno y vencimiento; la `returnUrl` queda en `/profesional/auth/register?billingReturn=1&checkoutRef=...` y puede conservar query params controlados del frontend como `mode=add-professional`; se valida contra el origen configurado cuando existe `BILLING_MERCADOPAGO_SUBSCRIPTION_BACK_URL`. Si Mercado Pago devuelve `checkoutUrl` pero todavia no devuelve `providerSubscriptionId`, responde igual `checkoutUrl`, `checkoutToken`, `checkoutRef`, `status=CHECKOUT_PENDING` y `confirmed=false`; el token lleva una referencia de registro firmada para verificar luego contra Mercado Pago. No crea profesional, suscripcion local ni activa Core en este paso.
- `POST /api/v1/billing/professional-registration/verify` — endpoint publico versionado para verificar el checkout contra Mercado Pago. Acepta `checkoutToken` y/o `checkoutRef`; con `checkoutRef` recupera el intent server-side aunque se haya perdido `sessionStorage`. Solo devuelve `confirmed=true` cuando la suscripcion remota esta `authorized`, `active`, `trial` o `trialing` y existe `providerSubscriptionId` real; no crea usuario ni perfil. Si el token inicial solo tenia referencia pendiente, puede devolver un `checkoutToken` renovado con el `providerSubscriptionId` confirmado. Si Mercado Pago sigue pendiente, rechazado, cancelado o vencido, actualiza el intent y devuelve estado diferenciado sin habilitar el alta profesional.
...
- `POST /auth/register/cliente` acepta `phoneVerificationToken` emitido por `/auth/register/phone/confirm` y lo exige cuando `AUTH_REGISTRATION_PHONE_VERIFICATION_REQUIRED=true`; con el default `false`, lo trata como opcional. El telefono cliente y el telefono profesional pueden ser distintos en la misma cuenta y se validan por contexto, no de forma global.
- `POST /auth/register/profesional` acepta `phoneVerificationToken` opcional si llega, pero no lo exige para el alta profesional; el telefono queda como contacto del perfil en `professional_profile.whatsapp`.
- Regla vigente de verificacion celular: cliente puede exigir OTP por flag (`AUTH_REGISTRATION_PHONE_VERIFICATION_REQUIRED=true`) y profesional guarda el telefono de contacto sin OTP obligatorio. El telefono verificable por `/auth/verify/phone/send` es `app_user.phone_number`; en alta/activacion profesional se persiste ahi el telefono de registro cuando falta, sin pisar `professional_profile.whatsapp`, y un `phoneVerificationToken` valido deja `phoneVerifiedAt` seteado. Temporalmente, para testing, `POST /public/profesionales/{slug}/reservas` no bloquea por telefono no verificado salvo que se active `APP_BOOKING_REQUIRE_PHONE_VERIFICATION=true`.
- `POST /auth/oauth/complete-phone` cierra el faltante de telefono cuando el alta/login OAuth no lo trae; exige `phoneVerificationToken` cuando `AUTH_REGISTRATION_PHONE_VERIFICATION_REQUIRED=true` y, con el default `false`, solo marca `phoneVerifiedAt` cuando el token valido llega.
...
- suggest con menor sobrelectura antes del ordenado final para bajar costo del typeahead
- `GET /api/search/suggest` ahora limita el match principal del dropdown a nombres utiles por tipo: categorias por `name/slug`, servicios por `service_name` y perfiles/locales por `display_name`; ya no debe promover `headline`, `about`, descripciones largas ni `locationText` como label principal
- `GET /api/search/suggest` mantiene `name` por compatibilidad y ahora tambien expone `displayName` + campos tipados (`professionalName`, `businessName`, `serviceName`) para que frontend priorice un label limpio sin romper clientes actuales
...
- `POST /profesional/services` exige email verificado por codigo en toda alta de servicio; si falta responde `403` con `Verificá tu email por código antes de crear servicios.`. La verificacion de telefono no bloquea el alta de servicios. `PUT` y `DELETE` de servicios existentes no aplican esta regla.
...
- `POST /cliente/reservas/{id}/payment-session` devuelve tambien `paymentBreakdown` y el `amount` del checkout ya sale del snapshot persistido en la reserva (`prepaid_total_amount_snapshot`), no de una cuenta repetida en frontend
...
- `GET /cliente/reservas`, `GET /cliente/reservas/proxima` y `GET /profesional/reservas` ahora exponen `paymentBreakdown` por reserva para que cliente y profesional vean `monto del servicio/seña`, `cargo de procesamiento` y `total cobrado` sin inferencias del cliente; el breakdown incluye ademas `processingFeeMode`, `% provider`, `% IVA` y `% plataforma` que originaron ese total snapshot-eado. En profesionales, `GET /profesional/reservas` tambien expone `price` desde `service_price_snapshot` para dashboards y detalle operativo sin depender del precio actual del servicio.
...
- `POST /api/v1/billing/professional-registration/checkout` inicia Mercado Pago para el wizard profesional sin requerir ni crear `ProfessionalProfile`; bloquea emails con profesional activo y puede devolver `checkoutUrl` aunque Mercado Pago todavia no haya entregado `providerSubscriptionId`, usando `checkoutRef` durable + referencia firmada pendiente para verificacion posterior. `verify` confirma contra Mercado Pago por `checkoutToken` y/o `checkoutRef` y exige `providerSubscriptionId` real antes de permitir alta profesional. `/auth/register/profesional` y `/auth/professional-profile/activate` vinculan el checkout confirmado y crean la suscripcion local `ACTIVE` en la misma transaccion que crea/activa `ProfessionalProfile`; `attach` queda como fallback/recovery autenticado.
...
- Mercado Pago crea `preapproval_plan` con `auto_recurring.free_trial` de `30 days` cuando no hay plan remoto configurado. En el checkout publico de registro profesional se puede devolver `checkoutUrl` sin `providerSubscriptionId`; el retorno/webhook debe actualizar el intent durable y verificar la referencia contra Mercado Pago. Solo con estado remoto confirmado y `providerSubscriptionId` real se crea la suscripcion local y el profesional; si el webhook llega antes de que exista `ProfessionalProfile`, solo confirma el intent y no crea suscripcion local.
