...
<<<<<<< HEAD
- `POST /api/v1/billing/professional-registration/checkout` — endpoint publico versionado para iniciar Mercado Pago desde el wizard profesional antes de crear cuenta/contexto/perfil profesional. Request: `planCode=PLAN_CORE`, `email`, `returnUrl?`. Si el email ya tiene perfil profesional activo devuelve `409` y no inicia Mercado Pago. Si Mercado Pago devuelve `checkoutUrl` pero todavia no devuelve `providerSubscriptionId`, responde igual `checkoutUrl`, `checkoutToken`, `status=CHECKOUT_PENDING` y `confirmed=false`; el token lleva una referencia de registro firmada para verificar luego contra Mercado Pago. No crea profesional, suscripcion local ni activa Core en este paso.
- `POST /api/v1/billing/professional-registration/verify` — endpoint publico versionado para verificar el `checkoutToken` contra Mercado Pago. Request: `checkoutToken`, `providerSubscriptionId?` opcional si Mercado Pago lo devuelve al volver del checkout. Solo devuelve `confirmed=true` cuando la suscripcion remota esta `authorized`, `active`, `trial` o `trialing` y existe `providerSubscriptionId` real; no crea usuario ni perfil. Si el token inicial solo tenia referencia pendiente, puede buscar la suscripcion por referencia aunque el email pagador de Mercado Pago sea distinto al email del wizard; como recuperacion si Mercado Pago no conserva esa referencia en el checkout hospedado, busca por email + plan si hay una unica suscripcion confirmada compatible y, si tampoco coincide el email, por plan + ventana temporal del `checkoutToken` solo cuando hay una unica suscripcion confirmada en esa ventana. Puede devolver un `checkoutToken` renovado con el `providerSubscriptionId` confirmado.
=======
- `POST /api/v1/billing/professional-registration/checkout` — endpoint publico versionado para iniciar Mercado Pago desde el wizard profesional antes de crear cuenta/contexto/perfil profesional. Request: `planCode=PLAN_CORE`, `email`, `returnUrl?`. Si el email ya tiene perfil profesional activo devuelve `409` y no inicia Mercado Pago. Crea un intent durable `professional_registration_checkout_intent` con `checkoutRef` opaco, email normalizado, plan, referencia de registro, plan remoto, URL de checkout, estado interno y vencimiento; la `returnUrl` queda en `/profesional/auth/register?billingReturn=1&checkoutRef=...` y se valida contra el origen configurado cuando existe `BILLING_MERCADOPAGO_SUBSCRIPTION_BACK_URL`. Si Mercado Pago devuelve `checkoutUrl` pero todavia no devuelve `providerSubscriptionId`, responde igual `checkoutUrl`, `checkoutToken`, `checkoutRef`, `status=CHECKOUT_PENDING` y `confirmed=false`; el token lleva una referencia de registro firmada para verificar luego contra Mercado Pago. No crea profesional, suscripcion local ni activa Core en este paso.
...
<<<<<<< HEAD
- `POST /api/v1/billing/professional-registration/checkout` inicia Mercado Pago para el wizard profesional sin requerir ni crear `ProfessionalProfile`; bloquea emails con profesional activo y puede devolver `checkoutUrl` aunque Mercado Pago todavia no haya entregado `providerSubscriptionId`, usando una referencia firmada pendiente para verificacion posterior. `verify` confirma contra Mercado Pago y exige `providerSubscriptionId` real antes de permitir alta profesional; los tokens nuevos duran `7` dias y se aceptan hasta `30` dias para recuperacion del retorno de Mercado Pago. `/auth/register/profesional` y `/auth/professional-profile/activate` vinculan el checkout confirmado y crean la suscripcion local `ACTIVE` en la misma transaccion que crea/activa `ProfessionalProfile`; `attach` queda como fallback/recovery autenticado.
=======
