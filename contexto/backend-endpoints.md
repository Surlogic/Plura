...
- `POST /api/v1/billing/professional-registration/verify` — endpoint publico versionado para verificar el `checkoutToken` contra Mercado Pago. Solo devuelve `confirmed=true` cuando la suscripcion remota esta `authorized`, `active`, `trial` o `trialing` y existe `providerSubscriptionId` real; no crea usuario ni perfil. Si el token inicial solo tenia referencia pendiente, puede devolver un `checkoutToken` renovado con el `providerSubscriptionId` confirmado.
...
- `POST /api/v1/billing/professional-registration/checkout` inicia Mercado Pago para el wizard profesional sin requerir ni crear `ProfessionalProfile`; bloquea emails con profesional activo y puede devolver `checkoutUrl` aunque Mercado Pago todavia no haya entregado `providerSubscriptionId`, usando una referencia firmada pendiente para verificacion posterior. `verify` confirma contra Mercado Pago y exige `providerSubscriptionId` real antes de permitir alta profesional. `/auth/register/profesional` y `/auth/professional-profile/activate` vinculan el checkout confirmado y crean la suscripcion local `ACTIVE` en la misma transaccion que crea/activa `ProfessionalProfile`; `attach` queda como fallback/recovery autenticado.
