...
- `POST /auth/register/profesional` conserva `phoneNumber` como telefono de contacto y acepta `phoneVerificationToken` si llega, pero ya no lo exige para el alta profesional; valida que el telefono no pertenezca a otra cuenta activa. Si el alta profesional viene desde Google, consume `oauthRegistrationToken` temporal emitido por `/auth/oauth` y recien en este submit final crea `UserRole.PROFESSIONAL` + `ProfessionalProfile`; antes de este punto no existe cuenta parcial. `POST /auth/oauth/complete-phone` mantiene la capacidad de guardar/verificar telefono OAuth y tambien valida unicidad de telefono activo.
