...
- `POST /auth/oauth` — login/registro OAuth Google/Apple. Para `authAction=REGISTER`, `desiredRole` es obligatorio (`USER` o `PROFESSIONAL`); si falta, responde `400` y no crea usuario. `authAction=LOGIN` mantiene compatibilidad y no exige `desiredRole`.
