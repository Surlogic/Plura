...
- `POST /profesional/services` exige email verificado solo cuando crea el primer servicio del profesional; si falta responde `403` con mensaje claro. La verificacion de telefono no bloquea el alta de servicios. `PUT` y `DELETE` de servicios existentes no aplican esta regla.
