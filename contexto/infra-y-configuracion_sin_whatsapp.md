...
- la secuencia real de migraciones Flyway del backend llega a `V72`; `V70__supabase_data_api_hardening.sql` crea `api_public` como schema expuesto vacio para PostgREST, revoca grants API sobre `public`, habilita RLS deny-all en tablas actuales de `public` y fija `search_path` en funciones SQL/PLpgSQL sensibles; `V71__advisor_index_cleanup.sql` limpia duplicados/redundancias marcadas por Supabase Advisor (`auth_refresh_token`, `professional_profile`, `booking`, `available_slot`, `auth_session`, `notification_event`, `app_product_event`, etc.) y agrega cobertura puntual adicional de billing/reviews; `V72__restore_fk_indexes_from_advisor.sql` repone los indices FK que Supabase siguio marcando como faltantes tras ese cleanup
