...
- los access tokens JWT legacy sin `sid/sv` quedan deshabilitados por default (`AUTH_ALLOW_LEGACY_JWT=false`); solo deben reactivarse durante una ventana de migracion controlada
...
- el registro por email cliente, el OTP SMS opcional de registro y la verificacion telefonica autenticada usan un proveedor OTP SMS configurable por `AUTH_PHONE_VERIFICATION_PROVIDER` (`vonage` o `firebase`); Vonage Verify se mantiene disponible y Firebase Phone Auth puede activarse por env sin cambiar endpoints. En `/auth/register/phone/send|confirm` y `/auth/verify/phone/send|confirm`, el backend persiste internamente el identificador del proveedor (`request_id` de Vonage o `sessionInfo` de Firebase) para mantener estable el contrato publico `telefono + codigo`. El registro cliente exige `phoneVerificationToken` cuando `AUTH_REGISTRATION_PHONE_VERIFICATION_REQUIRED=true`; con el default `false`, el OTP sigue siendo opcional y solo marca `phoneVerifiedAt` si llega un token valido. El registro profesional guarda telefono de contacto sin exigir OTP. Cuando un telefono queda verificado, no se reutiliza en otra cuenta activa
- despues de OAuth, el backend puede exigir completar telefono con `POST /auth/oauth/complete-phone`; ese cierre exige `phoneVerificationToken` cuando `AUTH_REGISTRATION_PHONE_VERIFICATION_REQUIRED=true`; con el default `false`, acepta el mismo token emitido tras Vonage Verify solo como verificacion opcional
...
`AUTH_REGISTRATION_PHONE_VERIFICATION_REQUIRED=true` vuelve obligatorio `phoneVerificationToken` en `POST /auth/register/cliente` y en `POST /auth/oauth/complete-phone`. El alta profesional mantiene el telefono como contacto y no exige OTP en ese flujo.
...
- `backend-java/fly.toml` usa `[deploy].strategy = "rolling"` y configura `wait_timeout = "15m"` porque el arranque real del backend en Fly puede tardar varios minutos mientras inicializa Flyway, JPA y validaciones de Spring. El healthcheck mantiene `grace_period = "60s"` porque Fly recorta valores mayores para checks HTTP.
...
- la secuencia real de migraciones Flyway del backend llega a `V93`; `V79__remove_internal_product_analytics.sql` elimina la tabla vieja `app_product_event`, `V80__rename_subscription_plan_contracts.sql` normaliza codigos legacy de planes si existe la tabla `subscription`, `V81__active_user_unique_identity.sql` ajusta unicidad de usuarios activos, `V82__subscription_core_trial_fields.sql` agrega columnas de trial/Core, `V83__normalize_subscription_plan_core.sql` normaliza `subscription.plan` a `PLAN_CORE`, `V84__subscription_plan_core_only.sql` elimina aliases legacy finales, `V85`/`V86` completan el tracking de verificacion telefonica, `V87__auth_session_active_context.sql` agrega contexto activo a sesiones, `V88__client_context_lifecycle.sql` agrega `app_user.client_active` para poder cerrar solo la faceta cliente, `V89__auth_session_active_context_backfill.sql` respalda el contexto activo, `V90__app_error_incidents.sql` crea incidentes internos de errores, `V91__create_billing_trial_claim.sql` agrega control anti-abuso de trials, `V92__context_phone_uniqueness.sql` ajusta unicidad telefonica por contexto activo y `V93__professional_registration_checkout_intent.sql` crea intents de checkout para registro profesional.
- las migraciones tempranas de busqueda que crean `public.immutable_unaccent()` ya no asumen que Supabase deje el diccionario como `public.unaccent`: resuelven el schema real de la extension `unaccent` (`public` o `extensions`) y generan la funcion apuntando a ese diccionario. Esto permite reconstruir una DB limpia desde `V9` sin depender de una reparacion posterior.
...
- `V70__supabase_data_api_hardening.sql` ya dejo `public` bloqueado para Data API (`authenticator -> pgrst.db_schemas=api_public`), con RLS y policy deny-all sobre tablas propias del proyecto en `public`; excluye `flyway_schema_history` porque Flyway la bloquea durante la migracion y habilitar RLS ahi puede cortar el arranque por `statement timeout`
- `V76__cleanup_unused_indexes_after_advisor.sql` no toca indices de `flyway_schema_history`; esa tabla es interna de Flyway y modificar sus indices durante el arranque puede cortar el deploy por `statement timeout` antes de que el backend abra `/health`
...
- billing de suscripciones requiere que el schema de `subscription` acepte solo `PLAN_CORE` como valor persistido; `V84` migra datos legacy desde `PLAN_BASIC`, `PLAN_PRO`, `PLAN_PROFESIONAL`, `PLAN_PROFESSIONAL`, `PROFESSIONAL`, `PLAN_PREMIUM`, `PLAN_LOCAL`, `LOCAL`, `PLAN_ENTERPRISE` y `ENTERPRISE` a `PLAN_CORE`, quedando como no-op seguro donde `subscription` o `subscription.plan` no exista
...
- `backend-java/fly.toml` expone `internal_port = 3000` y define `http_service.checks` sobre `GET /health` con `grace_period = 60s`; Fly recorta valores mayores en checks HTTP. El deploy fija `wait_timeout = 15m` para que `fly deploy` no corte antes de que la machine termine de pasar health cuando Spring tarda varios minutos en abrir Tomcat por DB, Flyway y JPA.
