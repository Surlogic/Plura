...
- `test` = integracion estable y rama del ambiente de prueba actual.
- las ramas cortas salen de `test`, salvo hotfixes urgentes.
...
1. Crear rama desde `test`.
2. Implementar el cambio con alcance acotado.
3. Abrir PR hacia `test`.
4. Pasar CI y revision.
5. Merge a `test`.
6. Cuando haya un grupo listo para salir, abrir PR `test -> prod`.
...
- preferir PRs chicos y squash merge para que el historial de `test` siga legible.

## Flujo normal para fixes no urgentes

1. Crear `fix/<descripcion>` desde `test`.
2. PR hacia `test`.
3. Sale a produccion en la proxima promocion `test -> prod`.
...
6. Inmediatamente despues, abrir PR de retorno `prod -> test` para que `test` no pierda el fix.

Regla:

- nunca dejar un hotfix solo en `prod`; siempre vuelve a `test`.

## Regla de deploy

- Backend de test actual: despliega desde `test` al Fly existente.
...
- `test` valida por CI y sigue siendo la rama de prueba operativa.
- Web productiva futura: debera configurarse tambien para desplegar desde `prod`.
- Antes de cada promocion `test -> prod`, revisar migraciones, variables nuevas, contratos y checklist de QA afectado.
...
2. Si puede esperar, usar `fix/*` desde `test`.
3. Si no puede esperar, usar `hotfix/*` desde `prod`.
4. Mantener el cambio minimo necesario.
5. Despues del deploy, devolver el cambio a `test`.
...
2. Proteger `test` con las mismas reglas base.
...
7. Configurar Vercel para que la rama de produccion sea `prod` y no `test` cuando se cree la web productiva.
8. Definir quien puede aprobar promociones `test -> prod` y hotfixes.

## Resumen operativo

```text
feature/* o fix/* -> test -> test actual
test -> prod -> produccion futura
hotfix/* desde prod -> prod -> produccion futura -> retorno a test
```
