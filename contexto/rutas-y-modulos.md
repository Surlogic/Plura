...
- `components/profesional`: UI publica y dashboard; el sidebar del dashboard muestra `Entrar como cliente` solo si `/auth/me` expone contexto `CLIENT`, selecciona `ctx=CLIENT` y redirige a `/cliente/inicio`.
...
- `/(auth)/complete-phone-client` completa el telefono faltante despues de OAuth via `POST /auth/oauth/complete-phone` sin OTP SMS; `/(auth)/complete-phone-professional` conserva el cierre de telefono del flujo profesional
...
- `ReservationAuthOverlay`: el registro embebido de cliente guarda telefono sin OTP SMS, respeta el `202 Accepted` opaco; ya no intenta login automático con la contraseña enviada, pasa a modo login con mensaje claro.
