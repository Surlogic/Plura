...
- `/profesional/auth/register` ahora usa un onboarding guiado por pasos en desktop: cuenta/Google, tipo de perfil, datos públicos con preview, rubros, modalidad, ubicación si aplica, horarios base, primer servicio y activación final de `Plura Core`. Mantiene `POST /auth/register/profesional` para email/password e inicia sesión automáticamente si puede, pero primero llama `POST /billing/subscription` con `PLAN_CORE`; solo cuando Core queda habilitado aplica el handoff de página pública/horarios/primer servicio. Google/OAuth completa teléfono/perfil y usa la misma regla. Si billing devuelve `checkoutUrl`, la web guarda pending checkout + handoff y redirige a Mercado Pago en la misma pestaña; hasta la confirmacion, las rutas operativas del dashboard redirigen a Facturación.
...
- `/profesional/auth/register`: el registro email/password profesional finaliza con `POST /auth/register/profesional`, intenta login automático con email/password, guarda el handoff inicial y activa `PLAN_CORE` antes de publicar página pública, agenda o primer servicio. Si el login automático falla después del alta, deriva a `/login?intent=professional&email=...&billing=pending`. Google/OAuth mantiene la sesión OAuth y usa la misma secuencia al finalizar el wizard.
...
- Wizard de registro profesional: el paso final transforma la configuración del wizard (descripción pública, agenda y primer servicio) a payloads reales de `/profesional/public-page`, `/profesional/schedule` y `/profesional/services`, pero el handoff solo se aplica después de que billing confirme Core como habilitado (`TRIALING`, `TRIAL` activo o `ACTIVE`). Mientras Core esté pendiente o falle, las rutas operativas del dashboard redirigen a Facturación.
