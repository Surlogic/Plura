...
- `/profesional/auth/register`: el registro email/password profesional finaliza con `POST /auth/register/profesional` y, como el backend responde `202 Accepted` sin sesión ni `accessToken`, redirige a `/profesional/auth/login?registered=1&email=...`. Google/OAuth mantiene la sesión OAuth y puede entrar al dashboard al finalizar el wizard.
