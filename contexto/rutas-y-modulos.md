...
- en `/explorar?vista=mapa`, el mapa prioriza `Mapbox GL` con estilo claro (`light-v11`) cuando WebGL esta disponible y cae a un fallback interactivo con Leaflet/CARTO Light cuando ese navegador no puede inicializar WebGL; si la URL no trae `lat/lng`, la vista intenta usar geolocalizacion exacta del navegador para centrar y relanzar la busqueda con `sort=DISTANCE` y radio inicial de `2 km`, sin inyectar antes el default general de `10 km`; si la URL ya trae `radiusKm`, ese valor se respeta como radio explicito. Si esa geolocalizacion vuelve con `accuracy > 1000m`, muestra un aviso discreto de ubicacion aproximada sin bloquear el mapa, y con `accuracy > 3000m` el buscador deja de rotularlo como `Cerca de mi`. La columna izquierda mantiene scroll propio y el mapa sigue siendo draggable/clickeable
