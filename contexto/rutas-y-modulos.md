...
- como se relacionan con `usuario`, `Profesional`, `Local` y `Enterprise`
...
- `/`, `/explorar` y las superficies de marketplace no deben mostrar profesionales sin servicios activos; el backend filtra esa regla en home, listados publicos, busqueda/suggest e hidratacion de resultados indexados.
- `/reservar`: antes de confirmar, el frontend bloquea clientes con `phoneVerified=false` y muestra mensaje claro; también valida el perfil cliente devuelto por el overlay de login/registro para no depender de estado viejo tras autenticarse; el backend también lo valida en `POST /public/profesionales/{slug}/reservas`.
- `/explorar/[slug]`: vista detallada de exploracion por slug.
- `/profesional/pagina/[slug]`: ruta publica canonica de la pagina del profesional/local. Con `?fromDashboard=1` se usa como revision acotada desde el dashboard profesional: mantiene la ficha publica, pero con sesion profesional el navbar solo permite volver al dashboard o cerrar sesion; el acceso directo sin query conserva la navegacion publica normal.
...
- `/login`: unico inicio de sesion real para cliente, profesional y trabajador; acepta `intent=client` o `intent=professional` y envia `desiredContext` desde el primer `POST /auth/login` o `POST /auth/oauth` para entrar directo a ese contexto cuando la cuenta lo tiene. El query legacy `billing=pending` no crea suscripcion ni abre Mercado Pago automaticamente; si falta Core despues de login profesional, deriva a la pantalla de billing para accion manual.
- `/oauth/callback`: callback de OAuth; para login Google conserva la intencion de `/login` y permite que `/auth/oauth` devuelva selector si no hay `intent`.
- `/profesional/auth/register`: registro profesional publico o wizard controlado para sumar contexto profesional. El botón Google solo verifica identidad y guarda una intención OAuth temporal; no autentica como cliente ni crea cuenta. Si el email Google ya existe solo como cliente, el wizard continua con `oauthRegistrationToken`. En el submit final primero inicia Mercado Pago con `POST /api/v1/billing/professional-registration/checkout` y guarda draft recuperable con `checkoutRef`; no llama `/auth/register/profesional` ni `/auth/professional-profile/activate` hasta que `POST /api/v1/billing/professional-registration/verify` confirma la suscripcion/trial/pago. El retorno de billing se procesa solo con `billingReturn=1`; cuando viene del CTA cliente conserva `mode=add-professional` para no redirigir por el contexto `CLIENT` activo mientras confirma Mercado Pago. `resume=1` solo retoma el wizard y no verifica Mercado Pago. Si vuelve `checkoutRef` en la URL, puede verificar contra backend aunque se haya perdido `sessionStorage`; si faltan datos locales del wizard, muestra recuperación para recompletar sin abrir Mercado Pago automáticamente. Si Mercado Pago sigue pendiente, muestra acciones manuales de reintento/volver a Mercado Pago. Recién con Mercado Pago confirmado crea o activa `PROFESSIONAL`; selecciona y persiste `ctx=PROFESSIONAL`, el backend adjunta la suscripcion local `ACTIVE` en esa misma operacion, la web verifica `/billing/subscription` antes de aplicar handoff inicial y redirige a la bienvenida profesional. Si falla el handoff inicial despues de activar Core, deriva a `/profesional/dashboard/billing?setup=pending` para recuperar desde el area profesional.
- `/profesional/auth/bienvenido`: pantalla post-alta profesional exitosa, mostrada después de Mercado Pago confirmado, Core activo leído por frontend y handoff aplicado; valida `ctx=PROFESSIONAL`, no bloquea por una falla aislada de hidratacion de perfil y mantiene CTA manual a `/profesional/dashboard`.
...
- home, explorar y paginas publicas de profesional usan metadata SEO compartida (`components/seo/SeoHead`) con title, description, canonical, Open Graph/Twitter y JSON-LD; no cambia la UX visible ni contratos de backend.
...
- `/profesional/pagina/[slug]` usa una landing publica mas ancha y continua: shell casi full-width (`max-w` alto + padding lateral corto), hero editorial mas abierto con direccion exacta, horarios, WhatsApp/telefono y redes integrados dentro del propio hero, galeria contenida arriba del catalogo, servicios en lista compacta, bloque `Sobre nosotros` debajo de servicios cuando existe `about/description` pública y luego una sección `Confianza y ubicación` en dos columnas con resumen de reseñas + modal y mapa/CTA a Google Maps; la galeria publica consume solo `photos` del perfil/negocio cargadas desde `presencia-publica` y no mezcla imagenes de servicios; el CTA principal `Reservar` ahora vive como botón flotante fijo abajo a la derecha y hace scroll suave a `#servicios`, mientras `Favoritos` queda dentro del hero y exige sesion cliente antes de tocar backend; ya no existe una seccion separada de `Ubicacion y horarios` ni se expone el email publico en la landing; la operacion completa de reserva queda derivada a `/reservar`. La implementacion real vive solo en `pages/profesional/pagina/[slug].tsx`.
...
- `/cliente/auth/complete-phone` completa el telefono faltante despues de OAuth via `POST /auth/oauth/complete-phone`; con el default actual no pide OTP SMS, y si backend activa `AUTH_REGISTRATION_PHONE_VERIFICATION_REQUIRED=true` debe enviar `phoneVerificationToken`. En profesional, el faltante se resuelve dentro del wizard de `/profesional/auth/register` y la ruta `complete-phone` solo redirige por compatibilidad.
- los formularios web que hoy piden telefono en auth (`/cliente/auth/register`, `/profesional/auth/register`, `/auth/forgot-password`, `/cliente/auth/complete-phone`, `/profesional/auth/complete-phone`) ya usan un selector de pais con bandera + codigo internacional y arman el `phoneNumber` final en formato internacional antes de enviarlo al backend; registro cliente publico, overlay de reserva y complete-phone cliente guardan el telefono sin OTP SMS con el default actual, pero deben adjuntar `phoneVerificationToken` si backend activa `AUTH_REGISTRATION_PHONE_VERIFICATION_REQUIRED=true`; el wizard profesional solo guarda el telefono como contacto del perfil. La variante `/cliente/auth/register?addContext=client` es activacion autenticada de contexto cliente y pide solo celular cliente sin OTP.
...
- `components/cliente`: shell y sidebar del area cliente; el sidebar muestra `Entrar como profesional` solo si `/auth/me` expone contexto `PROFESSIONAL`, selecciona `ctx=PROFESSIONAL` y redirige a `/profesional/dashboard`.
...
- `/cliente/auth/register` ya no pide OTP SMS ni escribir manualmente el prefijo internacional: usa selector de pais con bandera y compone el telefono completo para backend. Si ya hay sesion, no permite registro publico y redirige al dashboard del contexto activo. Si el email existe solo como profesional, el backend puede sumar contexto cliente al mismo usuario validando la password; si ya tiene cliente, bloquea con mensaje claro.
...
- `/cliente/auth/login`, `/cliente/auth/register` y el overlay de reserva ya no redirigen por `role=PROFESSIONAL`; fuerzan o seleccionan `ctx=CLIENT` cuando la intención es reservar o entrar al área cliente. El registro publico cliente no se usa para usuarios ya logueados. Con `?addContext=client`, `/cliente/auth/register` exige sesion profesional válida, muestra nombre/email desde `/auth/me` como datos bloqueados, pide solo celular cliente, llama `POST /auth/client-profile/activate`, selecciona `ctx=CLIENT` y redirige a `/cliente/inicio` o retoma `/reservar` si habia reserva pendiente. Sin sesion válida redirige a `/login?intent=client`; si ya tiene `CLIENT` redirige a `/cliente/inicio`.
...
- `/profesional/dashboard/servicios` — si el profesional no tiene servicios y le falta verificar email o telefono, muestra el bloque de verificacion antes del formulario; backend mantiene la autoridad y bloquea `POST /profesional/services` del primer servicio con `403` si falta alguna verificacion.
...
- `/profesional/dashboard/presencia-publica` — unifica `perfil-negocio` y `pagina-publica`: edita identidad, logo/banner, rubros, ubicación, contacto y redes contra `/profesional/profile`; edita mensaje principal, sobre el negocio y galería pública contra `/profesional/public-page`; mantiene preview en iframe, link público, QR y `Ver como cliente` hacia `/profesional/pagina/{slug}?fromDashboard=1`.
- `/profesional/dashboard/perfil-negocio` y `/profesional/dashboard/pagina-publica` — rutas legacy que cargan la pantalla unificada de `presencia-publica` para no romper enlaces internos existentes.
...
- `/profesional/auth/register`: si hay sesión base autenticada solo continúa cuando viene de flujo controlado (`mode=add-professional` desde dashboard cliente, `billingReturn=1` del checkout pendiente o `resume=1` desde login/onboarding); en ese caso activa/reactiva `ProfessionalProfile` con `POST /auth/professional-profile/activate`, guarda el telefono profesional en el perfil, selecciona `ctx=PROFESSIONAL` y continúa el onboarding. Si hay sesion y no viene de ese flujo, redirige al dashboard correspondiente.
- `/login?intent=professional`: usa login unificado con intención profesional y manda `desiredContext=PROFESSIONAL` en el primer request; si la cuenta valida todavía no tiene contexto profesional, autentica como `CLIENT` y retoma el onboarding profesional sobre esa misma cuenta.
...
- el login unificado expone `/auth/login` y permite que un mismo email use varios contextos (`CLIENT`, `PROFESSIONAL`, `WORKER`); el JWT lleva claim `ctx` con el contexto activo y `pid`/`wid` cuando aplica. Backend autoriza cliente por `ctx=CLIENT` si `app_user.client_active=true`, y autoriza profesional por `ctx=PROFESSIONAL` solo si hay `ProfessionalProfile.active=true`. Los wrappers legacy `/auth/login/cliente` y `/auth/login/profesional` ya delegan en la misma política de contexto, y web/mobile cliente/profesional usan `/auth/login` con `desiredContext`.
...
- `/profesional/dashboard/presencia-publica` incluye constructor visual para `logo` y `banner` dentro de un modal: se abre al terminar una subida/reemplazo y también desde `Editar encuadre`; ese encuadre queda persistido y se aplica también en la ficha pública
- los autocompletes de ubicacion en `/profesional/auth/register` y `/profesional/dashboard/presencia-publica` ya seleccionan sugerencias por click normal sin depender de `mouseDown`, evitando opciones que parecian clickeables pero no confirmaban bien al navegar con teclado o blur
- `/profesional/auth/register` conserva el onboarding profesional aunque la cuenta ya exista solo como cliente: el flujo publico requiere reautenticacion con la password de esa cuenta para sumar `PROFESSIONAL`; el flujo logueado requiere `mode=add-professional`/`resume=1`. El telefono del wizard es telefono profesional/comercial y se guarda como contacto del perfil sin OTP; no pisa el telefono cliente. El wizard valida disponibilidad contra `/auth/register/availability` con `desiredContext=PROFESSIONAL`, por lo que email solo cliente no bloquea y telefono duplicado solo bloquea contra profesionales activos.
- `/profesional/auth/register` funciona como wizard de alta: email/password no crea cuenta hasta el submit final de activacion Core; Google continua dentro del wizard, permite alta profesional nueva o sumar profesional sobre email solo cliente, el telefono se pide ahi mismo y si el usuario sale antes de terminar debe empezar otra vez. Al finalizar Google queda autenticado en `ctx=PROFESSIONAL` desde la respuesta de `/auth/register/profesional`, sin login por password. El registro y `/profesional/dashboard/presencia-publica` comparten selector internacional de telefono con bandera + codigo; evita cargar el prefijo a mano y deja el numero persistido listo para backend
- `/profesional/auth/register` ahora usa un onboarding guiado por pasos en desktop: cuenta/Google, tipo de perfil, datos públicos con preview, rubros, modalidad, ubicación si aplica, horarios base y activación final de `Plura Core`. La activación final es Mercado Pago primero: guarda draft local, inicia checkout publico versionado y, al volver, verifica estado real contra backend/Mercado Pago. Si Mercado Pago no confirma, no existe contexto `PROFESSIONAL`, no existe `ProfessionalProfile`, no se aplica handoff y el usuario queda en el wizard para reintentar. Con Mercado Pago confirmado, recién ahí usa `/auth/register/profesional` o `/auth/professional-profile/activate`; esos endpoints crean/activan el perfil y la suscripcion local `ACTIVE` de forma transaccional, y la web solo publica la configuración inicial despues de verificar esa suscripcion.
...
- `/profesional/dashboard/presencia-publica` sigue editando textos y galería, y la preview consume también el encuadre persistido de `logo` y `banner` definido en la misma pantalla
...
- `components/seo/SeoHead.tsx` y `lib/seo.ts`: metadata publica reutilizable para SEO, Open Graph, canonical y JSON-LD.
...
- `services/session.ts`: mantiene fallback token y un `known session hint` en storage para no disparar refresh/autofetch en rutas publicas cuando no hay sesion confirmada, y emite sincronizacion cross-tab por `BroadcastChannel` con fallback a `storage`.
...
- `pages/profesional/pagina/[slug].tsx`: compone la pagina publica con shell ancho (`max-w-[1500px]`), hero abierto con info util integrada (direccion exacta, horarios, WhatsApp/telefono y redes), botón flotante `Reservar` fijo abajo a la derecha con scroll interno a `#servicios`, toggle de `Favoritos` con guard client-side para anónimos, galeria contenida antes de servicios, sección `Sobre nosotros` usando `merged.about` cuando hay copy pública y cierre `Confianza y ubicación` con reseñas en modal + mapa o fallback de dirección; la galeria usa solo `merged.photos`/fotos publicas del negocio y excluye cualquier `imageUrl/photos` de servicios; ya no renderiza bloque aparte de `Ubicacion y horarios` ni email publico en esta landing. El backend devuelve `404` para perfiles sin servicios activos, por lo que no debe asumirse pagina publica visible hasta publicar al menos un servicio activo.
- `pages/reservar.tsx`: orquestador del flujo publico de reserva en `3` etapas reales; mantiene los mismos servicios/endpoints (`public profile`, `slots`, `create booking`, `payment-session`), compatibilidad con `pendingReservation`, sync de `serviceId/date/time/step` en URL y la derivacion a `/cliente/reservas` para seguimiento post-creacion/post-checkout. Mantiene verificacion de telefono antes de crear reserva o abrir Mercado Pago; si `create booking` responde `emailVerificationRequired=true`, muestra `EmailVerificationPanel` despues de crear la reserva y luego permite continuar a pago o seguimiento sin bloquear por email.
...
- `components/profesional/public-page/PublicProfileHero.tsx`: hero publico con banner ancho, logo superpuesto, rating y accesos sociales compactos en la misma fila (`WhatsApp` + redes disponibles como iconos circulares), favorito e info util simplificada bajo el banner; abajo quedan solo `Direccion` y `Horarios`, sin bloque `Sobre el profesional`, sin email publico y sin duplicar ubicacion.
- `components/profesional/public-page/PublicServicesSection.tsx`: lista compacta de servicios con pills por categoria, metadata breve, mas ancho horizontal y dos acciones fijas por item (`Ver detalle` + `Reservar`), sin estado visual de servicio preseleccionado en la landing; cada `Reservar` empuja el flujo dedicado con el `serviceId` del item clickeado.
...
- `/(auth)/complete-phone-client` completa el telefono faltante despues de OAuth via `POST /auth/oauth/complete-phone` sin OTP SMS con el default actual; si backend activa `AUTH_REGISTRATION_PHONE_VERIFICATION_REQUIRED=true`, debe enviar `phoneVerificationToken`. `/(auth)/complete-phone-professional` conserva el cierre de telefono del flujo profesional
...
- `/(auth)/register-client` y `/(auth)/register-professional` ya usan selector internacional de telefono con bandera + codigo y envian el numero final listo para backend; el registro cliente y `complete-phone-client` no piden OTP SMS con el default actual, pero deben enviar `phoneVerificationToken` si backend activa `AUTH_REGISTRATION_PHONE_VERIFICATION_REQUIRED=true`; el flujo profesional mantiene su regla propia
...
- responde bien al objetivo `Profesional` y `Local` de operar una agenda desde el telefono
...
- no expone aun el set completo de capacidades `Enterprise`
...
- `/profesional/auth/register`: el registro profesional usa wizard único para email y Google; el teléfono es obligatorio como contacto dentro del paso de datos básicos, pero ya no requiere OTP en este flujo. Google continúa el mismo wizard desde tipo de perfil y el frontend corta el loading OAuth si `/auth/oauth` o el cierre del callback queda pendiente, evitando que el registro quede cargando indefinidamente.
- `/profesional/auth/register`: el botón de Google del wizard profesional usa OAuth en la misma pestaña (`mode="redirect"`), persiste la intención `REGISTER/PROFESSIONAL` junto con el state PKCE, vuelve por `/oauth/callback` y continúa el mismo wizard sin popup. El teléfono sigue siendo obligatorio como dato de contacto dentro del paso de datos básicos.
...
- `/profesional/auth/register`: el registro email/password profesional inicia Mercado Pago solo desde el submit final del wizard y recién después de `verify` confirmado llama `POST /auth/register/profesional`, intenta login automático con email/password y guarda el handoff inicial. Si el login automático falla después del alta, deriva a `/login?intent=professional&email=...` sin disparar Mercado Pago automáticamente. Google/OAuth mantiene la sesión OAuth y usa la misma secuencia al finalizar el wizard.
- Backend `/auth/register/profesional`: si el email ya tiene contexto profesional activo, bloquea con conflicto contextual; si el email existe solo como cliente, exige password válida y suma `PROFESSIONAL` al mismo `app_user`.
- Backend `/auth/register/profesional`: valida rubros, modalidad, ubicación, coordenadas y teléfono profesional por contexto; en carrera de unicidad puede devolver conflicto controlado por email/contexto o teléfono profesional.
- `ReservationAuthOverlay`: el registro embebido de cliente guarda telefono sin OTP SMS con el default actual y debe completar verificacion telefonica antes del submit si backend activa `AUTH_REGISTRATION_PHONE_VERIFICATION_REQUIRED=true`; respeta el `202 Accepted` opaco, ya no intenta login automático con la contraseña enviada y pasa a modo login con mensaje claro.
- `/profesional/auth/register`: al completar Google OAuth en el wizard, se apaga `isGoogleLoading` antes de `setStep(1)` para evitar que `AuthLoadingOverlay` quede fijo si `GoogleLoginButton` se desmonta durante `onAuthenticated`. `GoogleLoginButton` también llama `onLoadingChange(false)` de forma explícita al finalizar/error.
...
- Wizard de registro profesional: el paso final transforma la configuración del wizard (descripción pública y agenda) a payloads reales de `/profesional/public-page` y `/profesional/schedule`, pero el handoff solo se aplica después de que billing confirme Core como habilitado (`TRIALING`, `TRIAL` activo o `ACTIVE`). Mientras Core esté pendiente o falle, las rutas operativas del dashboard redirigen a Facturación. La creación de servicios queda para el dashboard/onboarding operativo posterior.
...
- `/profesional/auth/login` queda como ruta legacy de compatibilidad y redirige a `/login?intent=professional`, preservando `email`, `registered` y otros query params; `billing=pending` queda como compatibilidad visual/legacy y no dispara checkout.
- `/login` sin intent muestra selector cuando la cuenta tiene varios contextos; mientras esa selección está pendiente guarda token sin `plura_auth_session_role`. `/login?intent=client` y `/login?intent=professional` seleccionan ese contexto automaticamente si existe y el contexto queda persistido en el access token.
- `/profesional/auth/register` es el destino de los CTAs publicos de adquisicion profesional como `Sumá tu negocio`; desde dashboard cliente el CTA `Quiero sumarme como negocio` usa `/profesional/auth/register?mode=add-professional` para sumar contexto profesional a la misma cuenta.
- `/login?intent=professional` queda para inicio de sesion profesional desde login unificado; si la cuenta no tiene contexto profesional, retoma el onboarding profesional sobre esa misma cuenta solo después de autenticar como cliente.
- Si el wizard profesional crea la cuenta pero falla el login automático, deriva a `/login?intent=professional`; al iniciar sesión como profesional, `/login` no activa `Plura Core` ni abre Mercado Pago por query params. Si Core no está habilitado, deriva a Facturación para acción manual.
