...
- `/profesional/pagina/[slug]` y `/profesional/[slug]` ahora usan una landing publica mas ancha y continua: shell casi full-width (`max-w` alto + padding lateral corto), hero editorial mas abierto, servicios en lista compacta, galeria contenida, bloque unificado de ubicacion/horarios y reseñas separados por divisores sutiles en vez de cards pesadas; la operacion completa de reserva queda derivada a `/reservar`
...
- `pages/profesional/pagina/[slug].tsx`: compone la pagina publica con shell ancho (`max-w-[1500px]`), hero abierto, CTA liviano hacia `/reservar`, servicios en lista compacta, galeria contenida, bloque unificado de ubicacion/horarios y reseñas separados por divisores; reutiliza el fetch cacheado del perfil publico y posterga geocoding fallback + mapa hasta que el bloque entra en viewport.
