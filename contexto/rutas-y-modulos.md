...
- `/explorar`: buscador principal con vistas `Páginas` y `Mapa`. En `Páginas`, navbar público + buscador + toolbar (`Páginas` / `Mapa` / `Ordenar`) quedan siempre visibles, el footer no se renderiza y el único scroll vertical vive dentro del contenedor de resultados/cards; `Mapa` conserva su layout previo. Las cards de resultados ya comparten la misma jerarquía visual del home (`banner + logo`, fallback a foto real del negocio, placeholder elegante si no hay assets).
