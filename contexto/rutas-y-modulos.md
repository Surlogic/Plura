...
- `/profesional/pagina/[slug]` y `/profesional/[slug]` ahora usan una landing publica mas ancha y continua: shell casi full-width (`max-w` alto + padding lateral corto), hero editorial mas abierto con direccion exacta, horarios, WhatsApp/telefono y redes integrados dentro del propio hero, galeria contenida arriba del catalogo, servicios en lista compacta, bloque `Sobre nosotros` debajo de servicios cuando existe `about/description` pública y luego reseñas; la galeria publica consume solo `photos` del perfil/negocio cargadas desde `pagina-publica` y no mezcla imagenes de servicios; el CTA principal `Reservar` ahora vive como botón flotante fijo abajo a la derecha y hace scroll suave a `#servicios`, mientras `Favoritos` queda dentro del hero y exige sesion cliente antes de tocar backend; ya no existe una seccion separada de `Ubicacion y horarios` ni se expone el email publico en la landing; la operacion completa de reserva queda derivada a `/reservar`
