...
- en `/explorar?vista=mapa`, el mapa prioriza `Mapbox GL` con estilo claro (`light-v11`) cuando WebGL esta disponible y cae a un fallback interactivo con Leaflet/CARTO Light cuando ese navegador no puede inicializar WebGL; si la URL no trae `lat/lng`, la vista intenta usar geolocalizacion del navegador con varias lecturas cortas (`watchPosition`, mejor `accuracy`, corte temprano si llega a precision util) antes de fijar la ubicacion y relanzar la busqueda con `sort=DISTANCE` y radio inicial de `2 km`, sin inyectar antes el default general de `10 km`; si la URL ya trae `radiusKm`, ese valor se respeta como radio explicito. Si esa geolocalizacion vuelve con `accuracy > 1000m`, muestra un aviso discreto de ubicacion aproximada sin bloquear el mapa, y con `accuracy > 3000m` el buscador deja de rotularlo como `Cerca de mi`; ademas ofrece `Actualizar mi ubicación` para forzar una lectura fresca del navegador aunque la URL ya tenga coordenadas, y `Ajustar ubicación` para arrastrar el mapa y confirmar `Usar centro del mapa` como zona elegida incluso si el navegador niega permisos o devuelve una lectura mala. Esa zona manual se persiste en `localStorage` (`plura:explore-manual-location`), se restaura antes de volver a pedir GPS si la URL viene limpia y, si el usuario limpia la ubicación desde los chips, se borran `lat/lng/radiusKm/locationSource` y no se reinyecta geolocalizacion automática en ese mismo mount. Cuando la geolocalizacion del navegador es solo contextual y hay filtros reales de búsqueda (query/categoría/ciudad/fecha), el viewport ya no se queda anclado en esa posicion incidental: prioriza encuadrar los resultados para no vaciar mapa/listado visible por desalineacion de zona. La columna izquierda mantiene scroll propio, sigue sin disparar búsquedas nuevas por cada pan/zoom y ya no filtra la lista por el cuadrante visible: muestra todos los resultados cargados, estilo marketplace tipo Booking/Airbnb, aunque alguno no tenga coordenadas o quede fuera del viewport actual. El popup del mapa quedo alineado entre Mapbox y Leaflet y ahora muestra tambien identidad `Local` vs `Perfil profesional`, ubicación y precio base cuando existen; el mapa sigue siendo draggable/clickeable
