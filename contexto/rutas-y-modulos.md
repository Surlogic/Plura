...
- `/profesional/dashboard/pagina-publica` — ahora incluye galería de fotos del negocio con upload vía `ImageUploader` (kind="gallery"), máximo Core según `maxBusinessPhotos=6`, preview en iframe; esas fotos son las que consume la galería pública del perfil y ya no se mezclan con imágenes de servicios; headline y about ya se editan también desde Core; el boton `Ir al sitio` abre `/profesional/pagina/{slug}?fromDashboard=1` en pestaña nueva para revision acotada, sin alterar el preview `?preview=1` ni el link/QR publico
