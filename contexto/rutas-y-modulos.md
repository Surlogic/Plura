...
- `/profesional/auth/register`: el registro email/password profesional finaliza con `POST /auth/register/profesional`, intenta login automático con email/password, refresca perfil, aplica el handoff inicial y activa `PLAN_CORE`. Si el login automático falla después del alta, deriva a `/profesional/auth/login?registered=1&email=...&billing=pending`. Google/OAuth mantiene la sesión OAuth y activa `PLAN_CORE` al finalizar el wizard.
