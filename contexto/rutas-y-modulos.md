...
- `/profesional/auth/register`: si hay sesión base autenticada, activa/reactiva `ProfessionalProfile` con `POST /auth/professional-profile/activate`, selecciona `ctx=PROFESSIONAL` y continúa el onboarding sin pedir otro email; la carga inicial de perfil/agenda queda guardada como handoff y se aplica solo cuando `Plura Core` queda habilitado. Los servicios se crean luego desde el dashboard/onboarding operativo.
...
- `/profesional/auth/register` conserva el onboarding profesional aunque la cuenta ya exista o haya sido creada por Google: si hay sesión profesional activa hidrata el wizard desde `/auth/me/profesional`, pide el teléfono dentro del flujo cuando Google no lo trae, lo guarda como telefono de contacto del perfil sin OTP dentro del wizard, mantiene draft local del progreso y al finalizar llama a backend para activar `Plura Core`; backend decide si corresponde prueba gratuita o checkout directo. El wizard valida disponibilidad contra `/auth/register/availability` antes de avanzar: email duplicado bloquea el paso Cuenta y telefono duplicado bloquea el paso Datos, sin cambiar la validacion final de `/auth/register/profesional` ni el flujo autenticado de activacion profesional sobre una cuenta cliente existente.
...
- Backend `/auth/register/profesional`: mantiene contrato anti-enumeración; si el email ya existe, quema work factor y responde 202 desde el controller sin revelar existencia.
- Backend `/auth/register/profesional`: valida rubros, modalidad, ubicación y coordenadas antes de consultar si el email existe para evitar side-channel de enumeración; en carrera de unicidad retorna opaco si detecta email duplicado y no devuelve 409.
...
- `/profesional/auth/register` es el destino de los CTAs publicos de adquisicion profesional como `Sumá tu negocio`.
- `/login?intent=professional` queda para inicio de sesion profesional desde login unificado; si la cuenta no tiene contexto profesional, retoma el onboarding profesional sobre esa misma cuenta.
