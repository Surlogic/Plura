...
- `/cliente/auth/login`, `/cliente/auth/register` y el overlay de reserva ya no redirigen por `role=PROFESSIONAL`; fuerzan o seleccionan `ctx=CLIENT` cuando la intención es reservar o entrar al área cliente. El registro publico cliente no se usa para usuarios ya logueados.
