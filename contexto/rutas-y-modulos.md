...
- `/profesional/pagina/[slug]` y `/profesional/[slug]` ahora usan una landing publica mas ancha y continua: shell casi full-width (`max-w` alto + padding lateral corto), hero editorial mas abierto con direccion exacta, horarios, WhatsApp/telefono y redes integrados dentro del propio hero, galeria contenida arriba del catalogo, servicios en lista compacta y luego reseñas; ya no existe una seccion separada de `Ubicacion y horarios` ni se expone el email publico en la landing; la operacion completa de reserva queda derivada a `/reservar`
...
- `pages/profesional/pagina/[slug].tsx`: compone la pagina publica con shell ancho (`max-w-[1500px]`), hero abierto con info util integrada (direccion exacta, horarios, WhatsApp/telefono y redes), CTA liviano hacia `/reservar`, galeria contenida antes de servicios y reseñas al final; ya no renderiza bloque aparte de `Ubicacion y horarios`, email publico ni mapa en esta landing.
