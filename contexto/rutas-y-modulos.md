...
- `/(auth)/register-client` y `/(auth)/register-professional` ya usan selector internacional de telefono con bandera + codigo, envian el numero final listo para backend y exigen OTP SMS antes de crear la cuenta; `complete-phone-client` y `complete-phone-professional` tambien lo piden al cerrar altas OAuth
...
- `ReservationAuthOverlay`: el registro embebido de cliente respeta el `202 Accepted` opaco; ya no intenta login automático con la contraseña enviada, pasa a modo login con mensaje claro.
