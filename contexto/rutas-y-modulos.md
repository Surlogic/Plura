...
- `/profesional/pagina/[slug]` y `/profesional/[slug]` ahora usan una landing publica mas ancha y continua: shell casi full-width (`max-w` alto + padding lateral corto), hero editorial mas abierto con direccion exacta, horarios, WhatsApp/telefono y redes integrados dentro del propio hero, galeria contenida arriba del catalogo, servicios en lista compacta y luego reseñas; la galeria publica consume solo `photos` del perfil/negocio cargadas desde `pagina-publica` y no mezcla imagenes de servicios; ya no existe una seccion separada de `Ubicacion y horarios` ni se expone el email publico en la landing; la operacion completa de reserva queda derivada a `/reservar`
