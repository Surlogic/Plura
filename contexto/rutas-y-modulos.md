...
- `/profesional/auth/register`: el registro email/password profesional finaliza con `POST /auth/register/profesional`, intenta login automático con email/password, guarda el handoff inicial y activa `PLAN_CORE` antes de publicar página pública o agenda inicial. Si el login automático falla después del alta, deriva a `/login?intent=professional&email=...&billing=pending`. Google/OAuth mantiene la sesión OAuth y usa la misma secuencia al finalizar el wizard.
