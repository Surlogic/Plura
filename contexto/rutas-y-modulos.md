...
- `/profesional/auth/register` conserva el onboarding profesional aunque la cuenta ya exista o haya sido creada por Google: si hay sesión profesional activa hidrata el wizard desde `/auth/me/profesional`, pide el teléfono dentro del flujo cuando Google no lo trae, lo guarda como telefono de contacto del perfil sin OTP dentro del wizard, mantiene draft local del progreso y al finalizar activa `Plura Core` con prueba gratuita. El wizard valida disponibilidad contra `/auth/register/availability` antes de avanzar: email duplicado bloquea el paso Cuenta y telefono duplicado bloquea el paso Datos, sin cambiar la validacion final de `/auth/register/profesional` ni el flujo autenticado de activacion profesional sobre una cuenta cliente existente.
