...
- `/profesional/auth/register` ahora usa un onboarding guiado por pasos en desktop: cuenta/Google, tipo de perfil, datos públicos con preview, rubros, modalidad, ubicación si aplica, horarios base y activación final de `Plura Core`. Mantiene `POST /auth/register/profesional` para email/password e inicia sesión automáticamente si puede, pero primero llama `POST /billing/subscription` con `PLAN_CORE`; solo cuando Core queda habilitado aplica el handoff de página pública/horarios. Google/OAuth completa teléfono/perfil y usa la misma regla. Si billing devuelve `checkoutUrl`, la web guarda pending checkout + handoff y redirige a Mercado Pago en la misma pestaña; si `trialPreviouslyUsed=true`, muestra copy mínimo de prueba ya utilizada antes de redirigir. Hasta la confirmacion, las rutas operativas del dashboard redirigen a Facturación. La carga de servicios queda fuera del registro y se resuelve después desde el dashboard/onboarding operativo.
